...
- (IBAction)login:(id)sender {
    if ([self.txtPhone.text isEqualToString:self.txtPhone.placeHolder]||self.txtPhone.text.length==0) {
        [[[UIAlertView alloc]initWithTitle:@"用户名不能为空" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil] show];
        return;
    }
    if ([self.txtPassword.text isEqualToString:self.txtPassword.placeHolder]||self.txtPassword.text.length==0) {
