...
-(void)setUI{
    UIColor *errorRed=[UIColor colorWithRed:230/255.0 green:57/255.0 blue:65/255.0 alpha:1];
    UIColor *editGray=[UIColor colorWithRed:102/255.0 green:102/255.0 blue:102/255.0 alpha:1];
    ...
    [[RACObserve(self, textType) map:^id(NSNumber *value) {
        ...
        if ([value integerValue]==loginTextTypeNormal) {
            ...
            [self.layer setBorderColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"输入边框"]].CGColor];
    ...
    [[RACObserve(self, text) map:^id(NSString *value) {
        if (value.length==0&&self.isFirstResponder==NO) {
            [self performSelector:@selector(setText:) withObject:self.placeHolder afterDelay:0];
        }else{
